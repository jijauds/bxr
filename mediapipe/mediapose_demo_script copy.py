...
for camera_angle in camera_angles:
    for move in move_list:
        frame_idx = 1
        output_csv   = Path(f"outputCsvs/{camera_angle}/{move}.csv")
        csv_file = open(output_csv, "w", newline="")
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(header)
        while True:
            image_path = Path(f"testData/{camera_angle}/{move}/{frame_idx}.png")
            ...
            output_image = Path(f"outputImages/{camera_angle}/{move}/{frame_idx}.png")
            #output_image = Path("outputImages/front/guard/1.png")
            ...
            print(f"Saved {output_image} and {output_csv}")
        csv_file.close()
